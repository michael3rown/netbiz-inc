var global_hooks = {
	...
	doc: $(document),
	body: $('body'),
	mobile: (/Mobile|iP(hone|od|ad)|Android|BlackBerry|IEMobile|Kindle|NetFront|Silk-Accelerated|(hpw|web)OS|Fennec|Minimo|Opera M(obi|ini)|Blazer|Dolfin|Dolphin|Skyfire|Zune/).test(navigator.userAgent.toLowerCase()) && 'ontouchstart' in document.documentElement
}
...
function clickfix(target, pane) {
	...
	cfix_target.on('click', function(e) {
		e.preventDefault();
		if (cfix_open === false) {
...
// contact form
contactform_handler.fn_default();
...
if (temp_width >= 780) {
	...
	if (temp_height > contactform_handler.form.outerHeight()) {
		// contact form
