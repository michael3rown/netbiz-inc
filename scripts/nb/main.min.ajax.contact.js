...
var contactform_handler = {
	link: $('#contact-link'),
	form: null,
	first: null, // $('#contact-form').find('input').first(),
	required: null, // $('#contact-form').find('input[required]'),
	close: null, // $('#contact-close'),
	overlay: null,
	bound: false,
	fetch: false,
	...
	fn_fetch: function() {
		$.ajax({
			...
			contactform_handler.fetch = true;

			contactform_handler.form = $('#contact-form');

			contactform_handler.form.html(msg);

			contactform_handler.first = contactform_handler.form.find('input').first();
			contactform_handler.required = contactform_handler.form.find('input[required]');
			contactform_handler.close = contactform_handler.form.find('#contact-close');
			contactform_handler.overlay = $('#contact-overlay');
			...
			// need to separate form check for contact page....
	...
	fn_bind: function() {
		if (!contactform_handler.bound) {
			contactform_handler.link.on('click', function(e) {
					e.preventDefault();
					if (!contactform_handler.fetch) {
						contactform_handler.fn_fetch();
					}
					else {
						contactform_handler.fn_toggle();
					}
			});
			contactform_handler.bound = true;
		}
	},
	...
	fn_unbind: function() {
		if (contactform_handler.bound) {
			...
			contactform_handler.bound = false;
		}
		contactform_handler.fn_hide();
	}
}
...
var stickynav_handler = {
	height: global_hooks.win.height(),
	position: 0,
	headerbar: $('#header-bar'),
	offset: $('#header-bar').outerHeight(),
	active: false,

	fn_calc: function(sn_pos, sn_height) {
		this.height = sn_height;
		this.fn_move(sn_pos);
	},

	fn_move: function(sn_pos) {
		this.position = sn_pos;
		if (this.position === 0) {
			this.headerbar.css({
				position: 'absolute',
			...
			this.headerbar.css({
				position: 'absolute',
				top: (this.height - this.offset) + 'px'
			}).addClass('hb-stickynav-scroll').removeClass('hb-stickynav-start hb-stickynav-stop');
		}
...
/* init */
var temp_width = global_hooks.win.width();
var temp_height = global_hooks.win.height();
var temp_fullscreen = global_hooks.body.hasClass('fullscreen');
var temp_contact = global_hooks.body.hasClass('page-contact');
...
if (temp_width >= 780) {
	...
	if (temp_fullscreen) {global_hooks.body.removeClass('fullscreen');}
}

/* scroll */
if (temp_fullscreen) {
	global_hooks.win.on('scroll', function() {
		if (stickynav_handler.active === true) {stickynav_handler.fn_move(global_hooks.win.scrollTop());}
	});
}

/* resize */
global_hooks.win.on('resize', function() {
	var temp_width = global_hooks.win.width();
	var temp_height = global_hooks.win.height();
